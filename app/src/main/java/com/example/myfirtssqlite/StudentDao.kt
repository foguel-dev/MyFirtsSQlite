...
class StudentDao(context: Context) : ContextWrapper(context) {
    ...
    private var dbHelpler: DbHelpler
    private var sqliteDatabase: SQLiteDatabase? = null

    init {
        dbHelpler = DbHelpler(context, DATABASE_NAME, DATABASE_VERSION)
    }
    ...
    fun insertStudent(name: String, faculty: String): Long {
        ...
        // insert into student (name,faculty) values(null,null)
        val cv = ContentValues()
        cv.put("name", name)
        cv.put("faculty", faculty)
    ...
    inner class DbHelpler(context: Context, name: String, version: Int) :
